...
class GroupsController extends AUserData {
	...
	public function updateGroup(string $groupId, string $key, string $value): DataResponse {
		...
		if ($key === 'displayname') {
			...
			$exists_name = array_reduce($groups, function ($acc, $group) use ($groupId, $value) {
				$displayname = $group->getDisplayName();
				$id = urldecode($group->getGID());
				if ($id !== $groupId && $displayname === $value) {
					array_push($acc, $group->getGID());
				}
				return $acc;
			}, []);
			if (count($exists_name) > 0) {
				$userId = $this->userSession->getUser()->getUID();
				$userLang = $this->config->getUserValue($userId, 'core', 'lang');
				$userLang = $userLang ? $userLang : 'vi';
				$message = $this->l10nFactory->get($this->appName, $userLang)->t('Group name already exists');
				throw new OCSException($message, 102);
			}
