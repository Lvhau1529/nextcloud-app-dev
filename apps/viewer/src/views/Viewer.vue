...
<template>
	...
	<NcModal
		...
		:style="{
			width: isSidebarShown ? `calc(100% - ${sidebarWidth}px)` : null,
		}"
	...
	>
		...
		<template #actions>
			...
			<!-- Menu items -->
			<NcActionButton
				v-if="Sidebar && sidebarOpenFilePath && !isSidebarShown"
				:close-after-click="true"
	...
	beforeMount() {
		...
		document.addEventListener("DOMContentLoaded", (event) => {
			...
			this.handlers.forEach((handler) => {
				if (handler.id === "text") {
					handler.mimes = handler.mimes.filter(
						(m) => !["application/xml", "text/xml"].includes(m)
					);
				}
	...
	methods: {
		...
		async onDelete() {
			try {
				...
				const url =
					this.source ?? this.root + this.currentFile.filename;
