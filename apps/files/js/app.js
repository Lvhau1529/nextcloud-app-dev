...
(function() {
	...
	OCA.Files.App = {
		...
		_onDirectoryChanged: function(e) {
			...
			// fix #9143: reset when change directory if search result empty
			const fileList = OCA.Files.App.fileList;
			if (fileList._filter && fileList.fileSummary.getTotal() < 1) {
				window._nc_event_bus.emit('nextcloud:unified-search.reset');
				const uniSearch = document.querySelector('form[role="search"]');
				if (uniSearch) {
					uniSearch.reset();
				}
			}
